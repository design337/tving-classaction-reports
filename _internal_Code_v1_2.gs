...
function _buildReportHtml(reportType, startDate, endDate, analysis, history) {
  ...
  html += '<section class="section"><div class="section-header">'
    ...
    + '<div class="callout"><span class="title">📌 핵심 메시지 — 실 결제자 중심</span>'
    + dateLabel + ' '
    + (payDay.completed > 0
        ? '<b>실 결제자 ' + _fmt0(payDay.completed) + '명</b>(' + _dod(payDay.completed, ((prev && prev._pay) ? prev._pay.completed : 0)) + ') · '
          + '<b>결제 CPA ₩' + _fmt0(adCost / payDay.completed) + '</b> · '
          + '<b>ROAS ' + (adCost > 0 ? (payDay.totalAmount / adCost * 100).toFixed(1) : '0') + '%</b> · '
          + '<b>신청→결제 전환 ' + ((totalAd + totalNonAd) > 0 ? (payDay.completed / (totalAd + totalNonAd) * 100).toFixed(1) : '0') + '%</b>. '
        : '<b>실 결제 데이터 없음</b>. ')
    + '광고비 ₩' + _fmt0(adCost) + '(' + _dod(adCost, prevAdCost) + ') · 클릭 ' + _fmt0(reportClicks) + '(' + _dod(reportClicks, (prev?prev.kpi.totalClk:0)) + ') · '
    + '신청자 ' + _fmt0(totalAd + totalNonAd) + '명*. '
    + '핵심 전환 채널 <span class="tag naver">네이버SA</span> ' + _fmt0(cls.naver) + '명, <span class="tag meta">메타</span> ' + _fmt0(cls.meta) + '명.'
    + '</div>'
    + '<div class="section-narrative">'
    + (payDay.completed > 0
        ? '<b>신청 → 결제 깔때기</b>: 신청자 ' + _fmt0(totalAd + totalNonAd) + '명 → 결제 ' + _fmt0(payDay.completed) + '명 (전환율 '
          + ((totalAd + totalNonAd) > 0 ? (payDay.completed / (totalAd + totalNonAd) * 100).toFixed(1) : '0') + '%, '
          + '이탈률 ' + ((totalAd + totalNonAd) > 0 ? (100 - payDay.completed / (totalAd + totalNonAd) * 100).toFixed(1) : '0') + '%). '
          + '<b>매출 ₩' + _fmt0(payDay.totalAmount) + '</b> · 결제 CPA ₩' + _fmt0(adCost / payDay.completed) + ' · ROAS '
          + (adCost > 0 ? (payDay.totalAmount / adCost * 100).toFixed(1) : '0') + '%. '
        : '결제 데이터가 아직 수집되지 않은 기간 — 신청자 기준 ' + _fmt0(totalAd + totalNonAd) + '명, CPA ₩'
          + _fmt0((totalAd + totalNonAd) > 0 ? adCost / (totalAd + totalNonAd) : 0) + '. ')
    + '내역(신청자): 광고 채널 ' + _fmt0(totalAd) + '명(네이버SA ' + _fmt0(cls.naver) + ' / 메타 ' + _fmt0(cls.meta) + '), '
    + '비광고 채널 ' + _fmt0(totalNonAd) + '명(카카오/오가닉/리퍼럴/AI/기타).<br/>'
    + '<span style="font-size:7pt;color:#94A3B8;">* 신청자는 GA 직접(direct) 채널 ' + _fmt0(directUsers) + '명 제외. 결제는 실결제자 시트 기준 (' + (isWeekly?'기간':'당일') + ' 결제 완료).</span>'
    + '</div></div></section>';
