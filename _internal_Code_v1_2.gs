 ...
 * Version: v1.1.2 (diag + header-detect + full-range zone1, 2026-06-16)
 *
 * 변경점 (v1.0 -> v1.1)
...
function doGet(e) {
  return _json({ok: true, info: 'Tving classaction reports', version: 'v1.1.2'});
}
...
function _logReportToSheet(reportType, startDate, endDate, url, title) {
  try {
    ...
    // 후보 매칭
    const candidates = {
      ts:    ['발행 일시', '발행일시', '발행일', '발행 일자', '발행시각', 'timestamp', '일시', '시각', '시간'],
    ...
    // 헤더 누락 시 fallback: 기존 default 컬럼 순서로 append
