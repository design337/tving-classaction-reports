 ...
 * Version: v1.1.6 (diag + header-detect + full-range zone1, 2026-06-16)
 *
 * 변경점 (v1.0 -> v1.1)
...
function doGet(e) {
  return _json({ok: true, info: 'Tving classaction reports', version: 'v1.1.6'});
}
...
function _aggregateByDate(ss, startDate, endDate) {
  ...
  // GA4_Raw — 사이트 전반 세션/참여율 — 채널×주차 가중 평균 이탈률/참여율 계산용 (v1.1.6)
...
function _writeZone1Fixed(ss, sheet, range) {
  ...
  // 가중 평균 참여율 — GA4_Raw 전체 세션 가중 (v1.1.6)
...
function _writeZone2Fixed(ss, sheet, range) {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    if (/소계/.test(ch) || ch === '전체 합계') {
      ...
      // 가중 평균 이탈률/참여율 — 채널별 또는 전체 (v1.1.6)
