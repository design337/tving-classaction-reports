 ...
 * Version: v1.1 (overview-sync + report-log-fix + ga-channel-expand, 2026-06-16)
 *
 * 변경점 (v1.0 -> v1.1)
...
function doPost(e) {
  try {
    ...
    if (action === 'inspect') return _json(_inspectSheets());
...
function doGet(e) {
  return _json({ok: true, info: 'Tving classaction reports', version: 'v1.1'});
}
...
function _aggregateByDate(ss, startDate, endDate) {
  ...
  _readSheetRows(ss, 'GA4_티빙 집단소송 신청하기').forEach(r => {
    ...
    else if (/direct|\(direct\)|\(none\)/i.test(sm)) ch = '직접';
...
function _logReportToSheet(reportType, startDate, endDate, url, title) {
  try {
    ...
    const headers = headerRange.map(h => String(h).trim());

    // 후보 매칭
    const candidates = {
      ts:    ['발행 일시', '발행일시', '발행일', '발행 일자', '발행시각', 'timestamp', '일시'],
      type:  ['보고서 유형', '보고서유형', '유형', '구분', '타입', 'type'],
      start: ['기간 시작', '기간시작', '시작일', '시작 날짜', '시작', 'start'],
      end:   ['기간 종료', '기간종료', '종료일', '종료 날짜', '종료', 'end'],
      title: ['제목', '보고서 제목', 'title'],
      url:   ['링크', 'URL', '주소', 'url'],
      status:['상태', 'status']
    };
    const findIdx = (cands) => {
    ...
    // 헤더 누락 시 fallback: 기존 단순 append
    if (idxTs < 0 && idxType < 0 && idxUrl < 0) {
      sheet.appendRow([new Date(), reportType, startDate, endDate, title, url, 'SUCCESS']);
      _appendRunLog('log_report', 'WARN', 'headers not matched, fallback append', JSON.stringify(headers).slice(0, 200));
      return;
    }

    // 헤더 매칭 row 작성
    const newRow = new Array(headers.length).fill('');
    if (idxTs    >= 0) newRow[idxTs]    = new Date();
    if (idxType  >= 0) newRow[idxType]  = reportType;
    ...
    _appendRunLog('log_report', 'SUCCESS', reportType + ' ' + startDate + '~' + endDate, url);
  } catch (err) {
    _appendRunLog('log_report', 'ERROR', String(err), err.stack || '');
  }
}
...
function _inspectSheets() {
    ...
    for (let r = 0; r < grid.length; r++) {
      ...
      for (let c = 0; c < grid[r].length; c++) {
        ...
        if (v !== '' && v !== null && v !== undefined) {
          rowOut[c] = (v instanceof Date) ? _fmt(v) : String(v).slice(0, 50);
        }
      }
  ...
  if (report) {
    ...
    const headers = report.getRange(1, 1, 1, lc).getValues()[0].map(h => String(h));
    out.report = {
...
function _buildReportFromSheet(body) {
  ...
  if (dates.length > 0) {
    try {
      const range = {dates: dates.slice().sort(), minDate: dates[0], maxDate: dates[dates.length-1], days: dates.length};
      overviewResult = _updateOverview(ss, range);
    } catch (err) {
      overviewResult = {ok: false, error: String(err)};
